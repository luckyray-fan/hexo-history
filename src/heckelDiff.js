var { diff, minimize, diffLines, diffWords, diffHybrid, refineChanged } = (function() {
  ...
  function minimize(changes) {
    var del = [],
      ins = [];
    return changes.concat({ type: 'same', eof: true }).reduce(function(acc, cur) {
      if (cur.type === 'del') {
        del.push(cur.value);
        return acc;
      }
  ...
  function diffLines(left, right, trim) {
    return diff(splitInclusive(left, '\n', trim), splitInclusive(right, '\n', trim));
  }

  function diffWords(left, right, trim) {
    return diff(splitInclusive(left, ' ', trim), splitInclusive(right, ' ', trim));
  }

  function diffHybrid(left, right, trim) {
    return refineChanged(diffLines(left, right, trim), function(del, ins) {
